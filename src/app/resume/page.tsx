...
import type { AiEvaluation, OptimizeResult, ResumeDiffChangeType } from "@/types/optimization";
...
const diffTypeLabels: Record<ResumeDiffChangeType, string> = {
  clarity: "Clarity",
  impact: "Impact",
  keyword: "Keyword",
  structure: "Structure",
...
function formatResultAsMarkdown(result: OptimizeResult) {
  ...
  return [
    ...
    "",
    "### Matched Strengths",
    ...result.jdMatchAnalysis.matchedStrengths.map((strength) => `- ${strength}`),
    "",
    "### Keyword Opportunities",
    ...(result.jdMatchAnalysis.missingKeywords.length
      ? result.jdMatchAnalysis.missingKeywords.map((keyword) => `- ${keyword}`)
      : ["No major keyword opportunities detected."]),
    "",
    "### Improvement Focus",
    ...result.jdMatchAnalysis.improvementFocus.map((focus) => `- ${focus}`),
    "",
    ...
    "",
    "### Changes",
    ...result.diff.changes.flatMap((change) => [
          ...
          <section aria-live="polite" className="min-w-0">
            ...
            {error ? (
              ...
              <div className="mt-6 space-y-5">
                ...
                <article className="rounded-lg border border-violet-200 bg-violet-50 p-5 shadow-sm sm:p-6">
                  <div className="border-b border-violet-200 pb-5">
                    <p className="text-sm font-semibold uppercase tracking-wide text-violet-700">
                      JD Match Analysis
                    </p>
                    <h3 className="mt-2 text-xl font-semibold text-slate-950">
                      Role fit and keyword opportunities
                    </h3>
                  </div>

                  <div className="mt-5 space-y-5">
                    <div className="rounded-md border border-violet-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">Matched Strengths</p>
                      <ul className="mt-3 space-y-2 leading-7 text-slate-700">
                        {result.jdMatchAnalysis.matchedStrengths.map((strength) => (
                    ...
                    <div className="rounded-md border border-violet-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">Keyword Opportunities</p>
                      {result.jdMatchAnalysis.missingKeywords.length ? (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {result.jdMatchAnalysis.missingKeywords.map((keyword) => (
                            <span
                              key={keyword}
                    ...
                    <div className="rounded-md border border-violet-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">Improvement Focus</p>
                      <ul className="mt-3 space-y-2 leading-7 text-slate-700">
                        {result.jdMatchAnalysis.improvementFocus.map((focus) => (
                          <li key={focus} className="break-words">
                            {focus}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="rounded-md border border-violet-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">Role Fit Summary</p>
                      <p className="mt-3 break-words whitespace-pre-wrap leading-7 text-slate-700">
                        {result.jdMatchAnalysis.roleFitSummary}
                      </p>
                    </div>
                  </div>
                </article>
                ...
                <article className="rounded-lg border border-sky-200 bg-sky-50 p-5 shadow-sm sm:p-6">
                  ...
                  <div className="mt-5 grid gap-4 xl:grid-cols-2">
                    <div className="rounded-md border border-sky-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">Before</p>
                      <p className="mt-3 break-words whitespace-pre-wrap leading-7 text-slate-700">
                        {result.diff.before}
                      </p>
                    </div>
                    <div className="rounded-md border border-sky-200 bg-white p-4">
                      <p className="text-sm font-semibold text-slate-950">After</p>
                      <p className="mt-3 break-words whitespace-pre-wrap leading-7 text-slate-700">
                        {result.diff.after}
                      </p>
                    </div>
                  ...
                  <div className="mt-5 space-y-3">
                    {result.diff.changes.map((change, index) => (
                      <div
                        key={`${change.type}-${index}`}
                        className="rounded-md border border-sky-200 bg-white p-4"
                      >
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <h4 className="font-semibold text-slate-950">Change {index + 1}</h4>
                          <span className="inline-flex w-fit rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-sky-700">
                            {diffTypeLabels[change.type]}
                          </span>
                        </div>
                        <div className="mt-4 grid gap-3 lg:grid-cols-2">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-slate-600">After</p>
                            <p className="mt-2 break-words whitespace-pre-wrap leading-7 text-slate-700">
                              {change.after}
                            </p>
                          </div>
